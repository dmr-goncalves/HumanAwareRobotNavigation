...
namespace social_navigation_layers
{
  ...
  /* First, we calculate which grid cell our point is in using worldToMapNoBounds.
  We
  Then we set the cost of that cell */
  void ProxemicLayer::updateCosts(costmap_2d::Costmap2D& master_grid, int min_i, int min_j, int max_i, int max_j){
    ...
    for(p_it = transformed_people_.begin(), i = 0; p_it != transformed_people_.end(), i < people_list_.people.size(); ++p_it, ++i){
      ...
      double cx = person.position.x, cy = person.position.y; // Detected Person Center Coordinates
